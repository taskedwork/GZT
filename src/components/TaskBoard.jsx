...
export default function TaskBoard() {
  ...
  const quadrants = useMemo(() => {
    ...
    return groups
  }, [tasks, filterStatus, state.mmEdges])
              ...
              >
                ...
                <div className="quadrant-header" style={{ background: q.bg }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: '.68rem', color: 'var(--muted)' }}>{q.desc}</span>
                    <span className="quadrant-count" style={{ background: q.color + '20', color: q.color }}>{items.length}</span>
                  </div>
                </div>
                ...
                <div className="quadrant-cards">
                  {items.length === 0 ? (
                    ...
                    items.map(task => {
                      ...
                      const projNode = state.mmNodes.find(n => {
                        return state.mmEdges.some(e => e.from === n.id && e.to === task.id)
                      })
                      const isDragging = dragTaskId === task.id
                      return (
                        <div
                          key={task.id}
                          className={`quadrant-card${isDragging ? ' quadrant-card-dragging' : ''}`}
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                            <button
                              ...
                              style={{ color: st.color, background: st.bg, width: 20, height: 20, fontSize: '.6rem' }}
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                            {projNode && (
                              <span style={{ fontSize: '.6rem', color: 'var(--muted)', background: 'var(--border)', padding: '1px 6px', borderRadius: 6 }}>
                                {projNode.label || projNode.name}
                              </span>
                            )}
                          ...
                          {/* 象限切换按钮 */}
                          <div className="quadrant-switch">
                            {QUADRANTS.filter(q2 => q2.id !== q.id).map(q2 => (
                              <button
                                key={q2.id}
                                className="quadrant-switch-btn"
                                style={{ color: q2.color, background: q2.color + '10' }}
                                onClick={() => moveQuadrant(task.id, q2.id)}
                                title={`移至${q2.label}`}
                              >
                                {q2.icon}
                              </button>
                          ...
                          {editModal && editModal.id === task.id && (
                            <div className="task-inline-edit" onClick={e => e.stopPropagation()}>
                              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
                                <input className="input" placeholder="待办名称" value={editName} onChange={e => setEditName(e.target.value)} style={{ flex: 1, minWidth: 100, fontSize: '.72rem' }} />
